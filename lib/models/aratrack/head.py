...
from lib.models.aratrack.utils import FrozenBatchNorm2d
...
def build_box_head(cfg):
    if cfg.MODEL.HEAD_TYPE == "MLP":
        ...
        if cfg.MODEL.HEAD_TYPE == "CORNER":
            ...
            inplanes = cfg.MODEL.HIDDEN_DIM + cfg.MODEL.HIDDEN_DIM // 2
            corner_head = Corner_Predictor(inplanes=inplanes, channel=channel,
                                           feat_sz=feat_sz, stride=stride, freeze_bn=freeze_bn)
        elif cfg.MODEL.HEAD_TYPE == "CORNER_UP":
            stride = 4
            feat_sz = int(cfg.DATA.SEARCH.SIZE / stride)
            inplanes = cfg.MODEL.HIDDEN_DIM # + cfg.MODEL.HIDDEN_DIM // 2
            corner_head = Pyramid_Corner_Predictor(inplanes=inplanes, channel=channel,
                                                   feat_sz=feat_sz, stride=stride, freeze_bn=freeze_bn)
        else:
            raise ValueError()
        return corner_head
