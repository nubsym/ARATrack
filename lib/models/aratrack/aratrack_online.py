...
from lib.models.aratrack.head import build_box_head
from lib.models.aratrack.utils import to_2tuple
from lib.utils.box_ops import box_xyxy_to_cxcywh, box_cxcywh_to_xyxy
from lib.models.aratrack.pos_utils import get_2d_sincos_pos_embed
from lib.models.aratrack.score_decoder import ScoreDecoder
from lib.test.utils.visual import pltshow
...
class Attention(nn.Module):
    ...
    def forward(self, x, t_h, t_w, s_h, s_w):
        ...
        attn_ts = (q_mt @ k_s.transpose(-2, -1)) * self.scale
        attn[:, :, :, :t_h * t_w * 2] = attn_ts.transpose(-2, -1) + attn[:, :, :, :t_h * t_w * 2]
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
...
class Block(nn.Module):
    ...
    def forward(self, x, t_h, t_w, s_h, s_w):
        x_att = self.attn(self.norm1(x), t_h, t_w, s_h, s_w)
...
class ConvViT(nn.Module):
    ...
    def forward(self, x_t, x_ot, x_s):
        ...
        # xs_shallow = self.weight_learn(x_s)
...
class ARATrackOnlineScore(nn.Module):
    """ ARATrack tracking with score prediction module, whcih jointly perform feature extraction and interaction. """
    ...
    def forward(self, template, online_template, search, run_score_head=True, gt_bboxes=None):
        ...
        out, outputs_coord_new = self.forward_head(search, run_score_head, gt_bboxes)

        return out, outputs_coord_new

    def forward_head(self, search, run_score_head=True, gt_bboxes=None):
        """
        ...
        if run_score_head:
            ...
            # (b,c,h,w) --> (b,h,w)
            out_dict.update({'pred_scores': self.score_branch(search, gt_bboxes).view(-1)})
...
def build_aratrack_online_score(cfg, settings=None, train=True) -> ARATrackOnlineScore:
    backbone = get_aratrack(cfg, train)  # backbone without positional encoding and attention mask
    score_branch = ScoreDecoder(pool_size=4, hidden_dim=cfg.MODEL.HIDDEN_DIM,
                                num_heads=cfg.MODEL.HIDDEN_DIM // 64)  # the proposed state-aware head
    box_head = build_box_head(cfg)  # a simple corner head
    model = ARATrackOnlineScore(
        backbone,
        box_head,
        score_branch,
    ...
    if cfg.MODEL.PRETRAINED_STAGE1 and train:
        ...
        if is_main_process():
            print("missing keys:", missing_keys)
            print("unexpected keys:", unexpected_keys)
            print("Loading pretrained aratrack weights done.")
