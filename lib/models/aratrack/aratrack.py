...
class Block(nn.Module):
    ...
    def forward(self, x, t_h, t_w, s_h, s_w):
        output = self.attn(self.norm1(x), t_h, t_w, s_h, s_w)
        x = x + self.drop_path1(output)
...
class ARATrack(nn.Module):
    ...
    def forward(self, template, online_template, search, run_score_head=False, gt_bboxes=None):
        ...
        out_dict, _ = self.forward_box_head(search)
        return out_dict, search  # attn_ts
...
def build_aratrack(cfg, train=True) -> ARATrack:
